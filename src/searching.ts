...
export function matchName(actualName: string, searchName?: string) {
  return (
    !searchName ||
...
export function matchTags(actualTags: string[] | undefined, searchTags: string[] | undefined) {
  ...
  for (const searchTag of searchTags) {
    if (searchTag.startsWith('!')) {
      if (actualTags && actualTags.includes(searchTag.substring(1))) {
      ...
      if (actualTags && actualTags.includes(searchTag)) {
...
export async function searchGenerators(
  ...
  for (const searchPath of [
    ...
    for (const directory of await fsListDirectories(searchPath)) {
      if (matchName(directory, listOptions.generator)) {
